/*
 * mobileSystem is based on droneSystem.cpp
 *
...
/* VERIFICAR FUNÇÃO loadTopics linha 204  */
...
namespace MOBILE {
	...
	*/
	...
	void System::loadTopics(ros::NodeHandle &n) {
		cmd_vel_publisher 		 = n.advertise<geometry_msgs::Twist>("/mobile/cmd_vel",1);
		transfPosition_publisher = n.advertise<nav_msgs::Odometry>("/mobile/transf_position",1);
		joy_subscriber 			 = n.subscribe<sensor_msgs::Joy>("/mobile/joy", 1, &System::joyCallback, this);
		odom_subscriber 		 = n.subscribe<nav_msgs::Odometry>("/mobile/odom", 1, &System::odomCallback, this);
		waypoint_subscriber 	 = n.subscribe<nav_msgs::Odometry>("/mobile/waypoint", 1, &System::waypointCallback, this);
		vicon_subscriber 	 	 = n.subscribe<geometry_msgs::TransformStamped>("/vicon/bebop/bebop", 1, &System::viconCallback, this);
//		fix_subscriber = n.subscribe<sensor_msgs::NavSatFix>("/mobile/fix", 10, &System::fixCallback, this);
	}
	...
	*/
	...
	void System::loadSettings(ros::NodeHandle &n) {
		...
		if (n.getParam("/mobile/vxAmpl",vxAmpl)) {
			cout << "vxAmpl = " << vxAmpl << endl;
		}

		if (n.getParam("/mobile/vyAmpl",vyAmpl)) {
			cout << "vyAmpl = " << vyAmpl << endl;
		}

		if (n.getParam("/mobile/vzAmpl",vzAmpl)) {
			cout << "vzAmpl = " << vzAmpl << endl;
		}

		if (n.getParam("/mobile/angAmpl",angAmpl)) {
			cout << "angAmpl = " << angAmpl << endl;
		}
		if (n.getParam("/mobile/f",f)) {
			cout << "f = " << f << endl;
		}

		int isEKFonline;
		if (n.getParam("/mobile/isEKFonline",isEKFonline)) {
			mobile.setIsEKFonline(isEKFonline);
			cout << "isEKFonline = " << mobile.getIsEKFonline() << endl;
		} else
		{
		...
		int updateRateEKF;
		if (n.getParam("/mobile/updateRateEKF",updateRateEKF)) {
			mobile.setUpdateRateEKF(updateRateEKF);
			cout << "updateRateEKF = " << mobile.getUpdateRateEKF() << endl;
		} else
		{
		...
		vector<double> K;
		if (n.getParam("/mobile/K",K)) {
			mobile.setK(Vector8d::Map(&K[0],8));
			cout << "K = [" << mobile.getK().transpose() << " ]" << endl;

		}

		vector<double> Kstart;
		if (n.getParam("/mobile/Kstart",Kstart)) {
			mobile.setKstart(Vector8d::Map(&Kstart[0],8));
			cout << "Kstart = [" << mobile.getKstart().transpose() << " ]" << endl;

		}

		vector<double> Kp;
		if (n.getParam("/mobile/Kp",Kp)) {
			Matrix4d KpMatrix;
			KpMatrix = KpMatrix.Zero();
			KpMatrix.diagonal() = Vector4d::Map(&Kp[0],4);
			mobile.setKp(KpMatrix);
			cout << "Kp = [" <<  mobile.getKp() << " ]" << endl;

		}

		vector<double> Kd;
		if (n.getParam("/mobile/Kd",Kd)) {
			Matrix4d KdMatrix;
			KdMatrix = KdMatrix.Zero();
			KdMatrix.diagonal() = Vector4d::Map(&Kd[0],4);
			mobile.setKd(KdMatrix);
			cout << "Kd = [" << mobile.getKd() << " ]" << endl;

		}

		vector<double> Ki;
		if (n.getParam("/mobile/Ki",Ki)) {
			Matrix4d KiMatrix;
			KiMatrix = KiMatrix.Zero();
			KiMatrix.diagonal() = Vector4d::Map(&Ki[0],4);
			mobile.setKi(KiMatrix);
			cout << "Ki = [" << mobile.getKi() << " ]" << endl;

		}

		vector<double> threshold;
		if (n.getParam("/mobile/threshold",threshold)) {
			mobile.setThreshold(Vector4d::Map(&threshold[0],4));
			cout << "INICIALIZANDO THRESHOLD COMO = " << mobile.getThreshold() << endl;
		}

		vector<double> inputRange;
		if (n.getParam("/mobile/inputRange",inputRange)) {
			mobile.setInputRange(Vector4d::Map(&inputRange[0],4));
			cout << "inputRange = " << mobile.getInputRange() << endl;
		}
		
		double amplitude;
		if (n.getParam("/mobile/amplitude",amplitude)) {
			setAmplitude(amplitude);
			cout << "Amplitude = " << amplitude << endl;
		}

		double velMed;
		if (n.getParam("/mobile/velMed",velMed)) {
			setVelMed(velMed);
			cout << "Vel. Media = " << velMed << endl;
		}

		string trajectory;
		if (n.getParam("/mobile/trajectory",trajectory)) {
		...
		if (n.getParam("/mobile/controlSelect",controlSelect)) {
			setControlSelect(controlSelect);
			cout << "controlSelect = " << controlSelect << endl;
		}

		string sensorSelect;
		if (n.getParam("/mobile/sensorSelect",sensorSelect)) {
	...
	*/
	...
	void System::bootVicon(const double& timeValue){
		if(sensorSelect.compare("VICON") == 0){
			...
			mobile.setTimeOrigin(timeValue);
	...
	*/
	...
	void System::control() {
	  ...
	  if(mobile.getIsEKFonline()){	  
	  	mobile.setIsFlying(flagEnable); 
	  }
	...
	*/
	...
	void System::odomCallback(const nav_msgs::Odometry::ConstPtr& odom)	{
		if(sensorSelect.compare("IMU") == 0){
			...
			positionLocal 	 = mobile.getPosition();
			orientationLocal = mobile.getOrientation();
	...
	*/
	...
	void System::waypointCallback(const nav_msgs::Odometry::ConstPtr& waypoint){
		...
		mobile.setPositionDesired(positionToGo);
		mobile.setYawDesired(yawDesired);

		mobile.setdPositionDesired(dpositionToGo);
		mobile.setdYawDesired(dYawDesired);
		
		mobile.setd2PositionDesired(d2positionToGo);
		mobile.setd2YawDesired(d2YawDesired);
	}
	...
	*/
	...
	void System::viconCallback(const geometry_msgs::TransformStamped::ConstPtr& vicon){
		...
		if(sensorSelect.compare("VICON") == 0){
			...
			timePast 		= mobile.getTimeNow(); 		//Stores last known time as past.

			time			= mobile.getThisTime(vicon->header.stamp.toSec()); // returns time in sec after start
			...
			mobile.setPosition(positionVicon);
			positionNow		= mobile.getPosition();
			linearVel 		= mobile.DvKalman(positionNow,time,timePast); //global terms instead of local (IMU)
			
			mobile.setOrientation(orientationVicon);
			orientationNow	= mobile.getOrientation();
			angularVel 		= mobile.DwKalman(orientationNow,time,timePast);

			/*Reset frame location*/
			if (!mobile.getIsOdomStarted()) {
				Conversion::quat2angleZYX(rpy,orientationVicon);
				yawVicon = rpy(2);
				mobile.setPosition0(positionVicon,yawVicon);
				mobile.setOrientation(orientationVicon);
			}
			
			mobile.setAngularVel(angularVel);
			mobile.setLinearVelVicon(linearVel);

			//This will reset coordinate frame and set initial time as zero
			if(!mobile.getIsViconStarted()){
				bootVicon(time); 
				mobile.setIsViconStarted(true);
			}

			mobile.setTimeNow(time); //Stores this time as current!
